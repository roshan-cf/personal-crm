...
export async function PUT(
  ...
  try {
    ...
    await db.execute({
      sql: `UPDATE contacts SET name = ?, relation = ?, remarks = ?, frequency = ?, category = ?, updated_at = datetime('now') WHERE id = ?`,
      args: [name, relation, remarks || null, frequency || 'weekly', category || 'friends', Number(id)],
    });
